...
def compute_config_for_logging(args) -> dict[str, str]:
...
def init_mlflow(args, *, primary: bool = True, **kwargs) -> None:
    ...
    if primary:
        init_mlflow_primary(args, experiment_name)
    else:
        init_mlflow_secondary(args)


def init_mlflow_primary(args, experiment_name: str) -> None:
    import mlflow
    ...
    mlflow.log_params(compute_config_for_logging(args))
...
def init_mlflow_secondary(args) -> None:
...
def log_metrics(metrics: dict[str, Any], step: int | None = None) -> None:
    ...
    for k, v in metrics.items():
        try:
            sanitized[sanitize_key(k)] = float(v)
